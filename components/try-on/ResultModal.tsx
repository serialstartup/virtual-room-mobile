...
const ResultModal: React.FC<ResultModalProps> = ({
  ...
  const [thumbsFeedback, setThumbsFeedback] = useState<FeedbackType | null>(
    null
  );
  ...
  const { downloadImage, isDownloading, isValidImageUrl } = useDownload();
  ...
  const {
    ...
    error: tryOnError,
    subscribeToUpdates: subscribeTryOnUpdates,
  } = useTryOnById(
    isAvatarProcessing || isTextToFashionProcessing ? "" : tryOnId || ""
  );
  const {
    avatar,
    isLoading: isAvatarLoading,
    error: avatarError,
    subscribeToUpdates: subscribeAvatarUpdates,
  } = useAvatarById(isAvatarProcessing ? tryOnId || "" : "");
  const {
    customModel,
    isLoading: isCustomModelLoading,
    error: customModelError,
    subscribeToUpdates: subscribeCustomModelUpdates,
  } = useCustomModelById(isTextToFashionProcessing ? tryOnId || "" : "");
  ...
  const isLoading = isAvatarProcessing
    ? isAvatarLoading
    : isTextToFashionProcessing
      ? isCustomModelLoading
      : isTryOnLoading;
  const error = isAvatarProcessing
    ? avatarError
    : isTextToFashionProcessing
      ? customModelError
      : tryOnError;
  const resultData = isAvatarProcessing
    ? avatar
    : isTextToFashionProcessing
      ? customModel
      : tryOn;
  const subscribeToUpdates = isAvatarProcessing
    ? subscribeAvatarUpdates
    : isTextToFashionProcessing
      ? subscribeCustomModelUpdates
  ...
  useEffect(() => {
    console.log("[RESULT_MODAL] 🔍 Effect triggered:", { 
      tryOnId, 
      visible, 
      isAvatarProcessing, 
      isTextToFashionProcessing 
    ...
    try {
      console.log("[RESULT_MODAL] 🔗 About to subscribe to updates:", { 
        hasSubscribeToUpdates: !!subscribeToUpdates,
        subscribeToUpdatesType: typeof subscribeToUpdates 
      });
      
      if (subscribeToUpdates) {
        unsubscribe = subscribeToUpdates((statusOrData: any) => {
          try {
            if (isAvatarProcessing) {
              // For avatar, the callback receives the full avatar object
              const avatarData = statusOrData as any;
              if (avatarData && typeof avatarData === 'object' && avatarData.status) {
              ...
              if (customModelData && typeof customModelData === 'object' && customModelData.status) {
              ...
              } else if (statusOrData && typeof statusOrData === 'object' && statusOrData.processing_status) {
                setCurrentStatus(statusOrData.processing_status as ProcessingStatus);
              }
            }
          } catch (error) {
            console.error("[RESULT_MODAL] ❌ Error in subscription callback:", error);
          }
        });
      }
    } catch (err) {
      console.error("[RESULT_MODAL] ❌ Error subscribing to updates:", err);
    }
    ...
    return () => {
      try {
        ...
        console.error("[RESULT_MODAL] ❌ Error unsubscribing:", err);
      }
    };
  }, [
    tryOnId,
    ...
    const autoSaveToWardrobe = async () => {
      if (
        ...
        try {
          ...
          console.error(
            "[RESULT_MODAL] ❌ Error auto-saving to wardrobe:",
            error
          );
  ...
  const handleThumbsFeedback = async (feedbackType: FeedbackType) => {
    ...
    try {
      ...
      const newFeedbackType = await toggleThumbsFeedback(
        tryOnId,
        workflowType,
        feedbackType
      );
                ...
                >
                  ...
                  >
                    <ThumbsUp
                      ...
                      fill={
                        thumbsFeedback === "like" ? "#22c55e" : "transparent"
                      }
                    />
                  </TouchableOpacity>
                  ...
                  >
                    <ThumbsDown
                      size={18}
                      color={
                        thumbsFeedback === "dislike" ? "#ef4444" : "#ef4444"
                      }
                      fill={
                        thumbsFeedback === "dislike" ? "#ef4444" : "transparent"
                      }
                    />
                  </TouchableOpacity>
                </View>
  ...
  return (
    ...
    >
      <View className="flex-1 bg-white">
          ...
          <Text className="text-2xl font-outfit-bold text-gray-800">
            {currentStatus === "completed"
              ...
              : currentStatus === "processing"
                ? t("tryOn.processing.processing")
                : currentStatus === "failed"
                  ? t("tryOn.processing.failed")
        ...
        {/* Content */}
        <View className="flex-1 p-4 justify-center">{renderContent()}</View>
      </View>
    </Modal>
  );
};
