...
class UserService {
  ...
  async updateUser(updates: UpdateUserRequest): Promise<User> {
    const response = await apiClient.put<ApiResponse<{ user: User }>>('/user/update', updates);
  ...
  async updateUserSettings(updates: UpdateUserSettingsRequest): Promise<UserSettings> {
    const response = await apiClient.put<ApiResponse<{ settings: UserSettings }>>('/user/settings', updates);
  ...
  async getNotificationSettings(): Promise<Pick<UserSettings, 'push_notifications' | 'email_notifications' | 'new_features'>> {
    const response = await apiClient.get<ApiResponse<{ notifications: Pick<UserSettings, 'push_notifications' | 'email_notifications' | 'new_features'> }>>('/user/settings/notifications');
  ...
  async getUserById(userId: string): Promise<User> {
    const response = await apiClient.get<ApiResponse<{ user: User }>>(`/user/${userId}`);
