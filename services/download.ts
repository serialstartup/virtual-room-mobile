...
export const downloadService = {
  ...
  async getDownloadInfo(
    ...
    try {
      ...
      if (response.success && response.data?.download_info) {
        console.log(
          `✅ [DownloadService] Download info retrieved:`,
          response.data.download_info
        );
      ...
      throw new Error(
        error.response?.data?.error || "Failed to get download info"
      );
    }
  },
  ...
  async getDownloadUrl(
    ...
    try {
      ...
      throw new Error(
        error.response?.data?.error || "Failed to get download URL"
      );
    }
  },
  ...
  async canDownload(tryOnId: string): Promise<boolean> {
    try {
      ...
      if (
        response.success &&
        typeof response.data?.can_download === "boolean"
      ) {
        console.log(
          `✅ [DownloadService] Download permissions checked:`,
          response.data.can_download
        );
      ...
      if (error.message?.includes("User not authenticated")) {
        console.warn(
          "[DownloadService] ⚠️ User not authenticated - cannot check download permissions"
        );
        return false;
      }

      console.error(
        "[DownloadService] Error checking download permissions:",
        error
      );
      return false;
    }
  ...
  async downloadImage(
    ...
    try {
      ...
      if (!canDownload) {
        Alert.alert(
          "Download Error",
          "You do not have permission to download this image"
        );
        return false;
      }
      ...
      console.log(
        `✅ [DownloadService] Image downloaded to:`,
        downloadResult.uri
      );
      ...
      // Track analytics
      analytics.trackShareImageClicked(tryOnId, "download");
  ...
  async shareImage(
    ...
    try {
      ...
      if (!canDownload) {
        Alert.alert(
          "Share Error",
          "You do not have permission to share this image"
        );
        return false;
      }
      ...
      // Track analytics
      analytics.trackShareImageClicked(tryOnId, "share");

      return true;
    } catch (err: any) {
      console.error("Error sharing image:", err);
      const error =
        err instanceof Error ? err : new Error("Failed to share image");
