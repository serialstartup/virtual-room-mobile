...
export const useWardrobe = () => {
  ...
  const favorites =
    wardrobeItems?.filter((item: WardrobeWithTryOn) => item.liked === true) ||
    [];
  ...
  const addToWardrobeMutation = useMutation({
    ...
    onSuccess: (newItem, { tryOnId }) => {
      // Track analytics
      analytics.trackWardrobeItemAdded("garment");
  ...
  const removeFromWardrobeMutation = useMutation({
    ...
    onSuccess: (_, tryOnId) => {
      // Track analytics
      analytics.trackWardrobeItemDeleted("garment");
  ...
  const updateWardrobeItem = async (
    tryOnId: string,
    liked?: boolean | null
  ) => {
